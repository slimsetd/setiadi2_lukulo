...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
  ...
  if ($setiadiForm->enableForm) {
    ...
    $array_edition = array('Pre Print', 'Revision', 'Published');
    ...
    $setiadiForm->createAnything('<label>Tanggal Pengesahan</label>'.simbio_form_element::dateField('isbn_issn', '').' <b>hh = Tanggal 01 s.d. 31; bb = Bulan 01 s.d. 12; tttt = Tahun contoh: '.date('Y'));
    // Publisher
    ...
    // Cover
    $str_image  = '<label>Cover Buku</label>';
    $str_image .= simbio_form_element::textField('file', 'image');
    $str_image .= ' Maximum '.$sysconf['max_image_upload'].' KB';
    $setiadiForm->createAnything($str_image);
    // Attachment
