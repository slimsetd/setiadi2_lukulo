...
class admin_logon
{
    ...
    public function adminValid($obj_db) {
        ...
        // modified by Drajat Hasan took from admin_logon.inc.php SLiMS 8 Akasia
        $_SESSION['upict'] = (empty($this->user_info['user_image']))?'person.png':$this->user_info['user_image'];
    ...
    protected function nativeLogin() {
        ...
        // password verify
        $verify_password = password_verify($this->password, $this->user_info['passwd']);
        if (!$verify_password) {
            $this->errors = 'Username or Password not exists in database!';
            return false;
        }
