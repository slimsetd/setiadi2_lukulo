...
class simbio_dbop extends simbio
{
    ...
    /**
     * Method to processing table based in array
     *
     * @param   array   $array_coloumn
     * @return  string
     */
    public function selectColoumn($array_coloumn = array())
    {
        if (!is_array($array_coloumn)) {
            return false;
        }
        $coloumn = "*";
        if (isset($array_coloumn) AND (count($array_coloumn) > 1) AND (!in_array('*', $array_coloumn))) {
            $coloumn = implode(', ', $array_coloumn);
        } else if ((count($array_coloumn) == 1) AND ($array_coloumn[0] != '*')) {
            $coloumn = $array_coloumn[0];
        }
    ...
    public function runQuery()
    {
        ...
        $outputQ = $this->obj_db->query("SELECT ".$this->selectColoumn($coloumn)." FROM ".$this->table." ".$this->criteria);
        $this->outputQuery = $outputQ;
    }

    /**
     * Method to get num_rows
     *
     * @param   object $fetch
     * @return  integer
     */
    ...
    {
        $this->num_rows = $this->outputQuery->num_rows;
    }
     ...
     * @return  result data
     */
    public function getData($fetch)
    {
        if ($this->outputQuery->num_rows > 0) {
            switch ($fetch) {
                case 'assoc':
                    $this->data = $this->outputQuery->fetch_assoc();
                    break;

                case 'object':
                    $this->data = $this->outputQuery->fetch_object();
                    break;
                
                default:
                    $this->data = $this->outputQuery->fetch_row();
                    break;
            }
        }
    }
